...
function setToArray(mySet) {
  ...
  let myArray = newArray.filter((value) => typeof value === "string");
...
function removeDuplicatesInPlace(arr) {
  ...
  firstSet.forEach((uniqueElement) => {
...
function areDisjoint(set1, set2) {
  ...
  let firstSet = new Set(set1);
  let secondSet = new Set(set2);
...
function iterateSet(set) {
  ...
  let newKeys = newSet.keys();

  for (let keys of newKeys) {
    console.log(keys);
  }

  let newValues = newSet.values();

  for (let values of newValues) {
    console.log(values);
  }

  let newEnt = newSet.entries();

  for (let iterators of newEnt) {
